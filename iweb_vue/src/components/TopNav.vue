<template>
  <b-container class="header">
    ...
    <div class="h_right">
      ...
      <div class="h_cart">
        <a href=""><em class="icon-cart"></em>我的购物车({{list.length}})<em class="icon-arrows"></em></a>
        <div class="cart_dropdown">
            
          <h3 v-show='$global.isLogin==false'>请<router-link to="login">登录</router-link>后查看您的购物车</h3>
          ...
          <ul v-show='$global.isLogin==true' class="list-unstyled">
            <li class="clearfloat" v-for="(item,index) in list" :key="index">
                <router-link :to="{name:'course_detail',query:{cid:item.courseId}}"><img :src="require(`@/assets/images/${item.pic}`)" alt=""></router-link>
                <dl>
                    <dt><router-link :to="{name:'course_detail',query:{cid:item.courseId}}">{{item.title}}</router-link></dt>
                    <dd>¥{{item.price}} <span>x{{item.count}}</span></dd>
                </dl>
                <em class="icon-remove" @click="delCart(index)"></em>
            </li>          
          </ul>
